...
#define FLOW_SCOPE { hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.tcp.srcPort, hdr.tcp.dstPort, hdr.ipv4.protocol }
//#define METADATA_OPERATION_COND (bit<32>) meta.l4Length
#define CUSTOM_ACTIONS_DEFINITION @name(".FlowBlaze.forward") \
                                  action forward() { \
                                    \
                                  } \
                                  @name(".FlowBlaze.fill_meta_flowlet_id") \
                                  action fill_meta_flowlet_id() { \
                                    meta.flowlet_id = (bit<16>)meta.flowblaze_metadata.R0; \
                                  }
#define CUSTOM_ACTIONS_DECLARATION forward; fill_meta_flowlet_id;
